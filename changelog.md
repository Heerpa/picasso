...
- Added spline PSF calibration generation from a bead z-stack (new module `picasso.spline`): detects beads, registers them to a common center in 3D by cross-correlation and iteratively averages them into a clean PSF volume, rejecting dissimilar beads (following Li et al., Nat. Methods 2018; multiple fields of view per z position are supported, mirroring 3D astigmatism calibration), normalizes it and computes cubic-spline coefficients with Gpuspline. Available in Localize under **3D > Calibrate spline PSF**, or on the command line via `picasso spline-calibrate <bead_movie> -s <z_step_nm>`.
