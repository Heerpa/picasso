...
- Added the backend architecture for cubic-spline PSF fitting on the GPU (GPUfit's `SPLINE_2D`/`SPLINE_3D` models), fitting an experimentally measured PSF instead of a Gaussian. A 3D spline calibration recovers `z` directly in a single fit (no separate astigmatism z-fitting). The `pygpuspline` binding (used to build the spline coefficients) is vendored under `picasso/ext/pygpuspline`; its compiled library must be supplied by the user, like `Gpufit.dll`. Fitting method code `"spline-gpu"` in `localize.fit2D`/`localize`/`localize_3D`, with calibrations stored as HDF5 via `io.save_spline_calibration`/`load_spline_calibration`. GUI/CLI fit wiring is coming in a follow-up update.
- Added spline PSF calibration generation from a bead z-stack (new module `picasso.spline`): detects beads, averages them into a laterally-centered PSF volume (multiple fields of view per z position are supported, mirroring 3D astigmatism calibration), normalizes it and computes cubic-spline coefficients with Gpuspline. Available in Localize under **3D > Calibrate spline PSF** (requires the Gpuspline library; the coefficient step runs on the CPU).
