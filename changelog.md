...
- **Standalone channel registration** (`Calibration` > `Register channels (2D)`, new module `picasso.registration`), which measures where each loaded channel sits relative to the first and saves it as its own small `.yaml`. It can be built either **from bead images** or **directly from the experimental blinking signal** where brightest blinks are recommended.
