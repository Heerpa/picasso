...
- New fitting model: **Experimental PSF (cubic spline)** — fits an experimentally measured PSF (a cubic-spline model built from a bead z-stack) on the GPU, via GPUfit's `SPLINE_2D`/`SPLINE_3D` models, with least-squares or maximum-likelihood estimators. A 3D spline calibration recovers `z` directly in a single fit (no separate astigmatism z-fitting). Choose it in the Localize *Fit Settings* and load a calibration under *Experimental PSF (spline)*; on the command line use `--fit-method spline` / `spline-mle` with `--spline-calibration <file.hdf5>`. The `pygpuspline` binding is vendored under `picasso/ext/pygpuspline`; the compiled Gpufit/Gpuspline libraries must be supplied by the user, like `Gpufit.dll`. Calibrations are stored as HDF5 (`io.save_spline_calibration`/`load_spline_calibration`).
- Added spline PSF calibration generation from a bead z-stack (new module `picasso.spline`): detects beads, registers them to a common center in 3D by cross-correlation and iteratively averages them into a clean PSF volume, rejecting dissimilar beads, then regularizes it (background removed by subtracting the stack minimum and denoised along z with a smoothing B-spline) before normalizing to unit peak (following Li et al., Nat. Methods 2018; multiple fields of view per z position are supported, mirroring 3D astigmatism calibration) and computing cubic-spline coefficients with Gpuspline. Available in Localize under **3D > Calibrate spline PSF**, or on the command line via `picasso spline-calibrate <bead_movie> -s <z_step_nm>`.
