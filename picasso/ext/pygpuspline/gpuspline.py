...
Vendored into Picasso unchanged from upstream. Picasso uses spline_coefficients
to build cubic-spline PSF calibrations that are then fitted with Gpufit's
SPLINE_2D / SPLINE_3D models (see picasso.localize). The compiled library
(splines.dll on Windows, libsplines.so on Linux) is NOT shipped and must be
placed next to this file; see README.txt.
"""
...
if os.name == "nt":
    lib_path = os.path.join(
        package_dir, "splines.dll"
    )  # library name on Windows
elif os.name == "posix":
    lib_path = os.path.join(
        package_dir, "libsplines.so"
...
def spline_coefficients(data):
    ...
    coefficients = np.zeros(
        [number_coefficients_per_interval] + intervals, np.float32
    )
    ...
    if status != 0:
        raise RuntimeError(
            "Call to Gpuspline library failed, exit code {}".format(status)
        )
...
def spline_interpolate(data, x, y=None, z=None):
    ...
    interpolated_size = [new_size_x, new_size_y, new_size_z][
        :number_dimensions
    ]
