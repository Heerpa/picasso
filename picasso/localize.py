...
def _spline_link_xyz_crlb_cpu(
    ...
    localizations the device could not diagonalize). Returns the raw
...
def _spline_link_xyz_crlb(
    ...
    :func:`_spline_infomats_link_xyz_3d`). ``em`` applies the same EMCCD
    excess-noise doubling. Returns ``(n_locs, 3 + 2*n_channels)`` variances in
    that parameter order."""
    theta = np.asarray(theta, dtype=np.float64)
    n_channels = _spline_n_channels(calibration)
...
def _spline_crlb_cpu(
    ...
    device could not diagonalize). ``z_eval`` None selects the 2D model. Returns
    the raw ``(n_locs, P)`` covariance diagonal; masking non-finite and
    non-positive entries is the caller's job."""
    is_3d = z_eval is not None
    n_params = 5 if is_3d else 4
    n_locs = len(amplitude)
