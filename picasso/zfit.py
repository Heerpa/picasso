...
def zfit(
    ...
    *,
    ...
    fitting_method: Literal["gausslq", "gaussmle"] = "gausslq",
    filter: int = 2,
    lateral_transforms: dict | list | str | None = None,
    ...
    lateral_transforms : dict, list, str or None, optional
        Lateral (x, y) corrections held *outside* `calibration`, e.g. a
        standalone chromatic-aberration calibration kept separate from
        the astigmatism one. A calibration dictionary carrying a
        ``"Lateral transforms"`` list, the list itself, or the path of a
        calibration file to read them from. They are applied after the
        ones `calibration` carries, in list order, so loading a correction
        separately gives the same coordinates as appending it to the 3D
        calibration with ``lib.append_lateral_transform``. A correction
        `calibration` already carries is skipped (with a
        ``lib.DuplicateLateralTransformWarning``) rather than applied
        twice; both lists are reported in the returned metadata, under
        ``"Lateral corrections applied"`` and ``"Lateral corrections
...
def _zfit(
    ...
    # two sources are simply concatenated in that order. One that is in
    # both counts once - applying it twice would correct twice.
    own = lib.lateral_transforms(calibration)
    extra, duplicates = lib.drop_duplicate_lateral_transforms(
        lib.resolve_lateral_transforms(lateral_transforms), own
    )
    if duplicates:
        warnings.warn(
            "Skipping "
            + ", ".join(lib.describe_lateral_transforms(duplicates))
            + ": the 3D calibration already carries this lateral correction "
            "and applies it itself. Applying it again would correct the "
            "coordinates twice.",
            lib.DuplicateLateralTransformWarning,
            stacklevel=3,
        )
    affine = lib.describe_lateral_transforms(own + extra)
    locs = lib.apply_lateral_transforms(locs, own + extra)
    new_info = {
    ...
    if duplicates:
        new_info["Lateral corrections skipped"] = (
